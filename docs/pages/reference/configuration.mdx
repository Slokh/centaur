...
| `apiRs.applicationGateway.*` | Optional execution-bound gateway to operator-owned application capabilities. |
...
| `sandbox.prewarmTools`, `sandbox.prewarmTimeoutSeconds` | Optional tool CLIs materialized before sandbox readiness, plus the per-tool timeout. |
...
| `apiRs.applicationGateway.*`, `CENTAUR_APPLICATION_URL`, `CENTAUR_APPLICATION_CAPABILITIES`, `CENTAUR_APPLICATION_SIGNING_SECRET` | Helm values and `secretManager.existingSecretName`; disabled by default. | Exposes only the named operator-owned application capabilities to the sandbox that owns the active execution. |
| `CENTAUR_APPLICATION_AUTHORITY_MAX_AGE_SECONDS` | `apiRs.extraEnv`; default `900`. | Maximum age of the trusted member invocation accepted for a mutating application capability. |
...
| `SESSION_SANDBOX_PRIORITY_CLASS_NAME` | `sandbox.priorityClassName`. | Optional PriorityClass applied to runtime-created sandbox and paired iron-proxy pods. Set it below `global.priorityClassName` when control-plane workloads should preempt disposable execution capacity. |
...
| `CENTAUR_TOOL_PREWARM`, `CENTAUR_TOOL_PREWARM_TIMEOUT_SECONDS` | `sandbox.prewarmTools`, `sandbox.prewarmTimeoutSeconds`. | Comma-separated tool CLIs invoked with `--help` after shim installation and before readiness. A configured tool that is missing, fails, or times out prevents the sandbox from becoming ready. |
