...
| `sandbox.prewarmTools`, `sandbox.prewarmTimeoutSeconds` | Tool CLIs that must successfully materialize their environments before sandbox readiness, plus the per-tool timeout. |
...
| `SESSION_SANDBOX_PRIORITY_CLASS_NAME` | `sandbox.priorityClassName`. | PriorityClass applied to runtime-created sandbox and paired iron-proxy pods. Set it below `global.priorityClassName` so control-plane workloads can preempt disposable execution capacity. |
...
| `CENTAUR_TOOL_PREWARM`, `CENTAUR_TOOL_PREWARM_TIMEOUT_SECONDS` | `sandbox.prewarmTools`, `sandbox.prewarmTimeoutSeconds`. | Comma-separated tool CLIs invoked with `--help` after shim installation and before readiness. Missing, failed, or timed-out configured tools fail sandbox startup. |
