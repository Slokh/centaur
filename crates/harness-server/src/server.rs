...
const SESSION_ENV_ALLOWLIST: [&str; 2] = ["CENTAUR_THREAD_KEY", "CENTAUR_APPLICATION_GATEWAY_KEY"];
...
mod tests {
    ...
    fn parses_only_allowlisted_private_session_env() {
        let line = r#"{"type":"user","text":"hi","session_env":{"CENTAUR_THREAD_KEY":"discord:G:C:M","CENTAUR_APPLICATION_GATEWAY_KEY":"bound-key","PATH":"/attacker","EMPTY":""}}"#;
        let BlocksCommand::User { session_env, .. } = parse_blocks_line(line).expect("parses")
        else {
            panic!("expected user command");
        };

        assert_eq!(session_env.len(), 2);
        assert_eq!(
        ...
        assert_eq!(
            session_env
                .get("CENTAUR_APPLICATION_GATEWAY_KEY")
                .map(String::as_str),
            Some("bound-key")
        );
