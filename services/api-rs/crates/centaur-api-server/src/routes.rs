...
use crate::{
    ...
    types::{
        ...
        ListWorkflowRunsQuery, MutationAuthority, OnHarnessConflict, SessionContextResponse,
...
const MAX_APPLICATION_RESPONSE_BYTES: usize = 1024 * 1024;
const MAX_APPLICATION_CAPABILITY_LEN: usize = 128;
...
mod route_access_tests {
    use super::{Method, RouteAccess, route_access};
...
async fn create_or_get_session(
    ...
    let metadata = session_metadata_with_chat_destination(
        request.metadata,
        request.chat_destination.as_ref(),
    )?;
    let outcome = runtime
...
async fn get_session_context(
    ...
    resolve_session_context(state, caller, raw_thread_key, None).await
}
...
async fn resolve_session_context(
    ...
    if let Some(execution_metadata) = execution_metadata {
        destination = scoped_destination_for_execution(destination, execution_metadata);
    }
...
async fn get_scoped_session_context(
    ...
    let thread_key = ThreadKey::try_from(raw_thread_key)?;
    let runtime = state.runtime()?;
    authorize_exact_principal_session_owner(&runtime, &caller, &thread_key).await?;
    let execution = runtime
        .active_execution(&thread_key)
        .await?
        .ok_or_else(|| ApiError::Forbidden("no active execution for this session".to_owned()))?;
    let invocation = invocation_from_execution_metadata(&execution.metadata)?;
    validate_scoped_transport_authority(&invocation)?;
...
async fn invoke_application_capability(
    ...
    let thread_key = ThreadKey::try_from(raw_thread_key)?;
    authorize_exact_principal_session_owner(&state.runtime()?, &caller, &thread_key).await?;
    validate_application_capability_name(&capability)?;
    ...
    loop {
        ...
        if bytes.len().saturating_add(chunk.len()) > MAX_APPLICATION_RESPONSE_BYTES {
            let _ = state
                .runtime()?
                .append_application_event(
                    &thread_key,
            ...
            return Err(ApiError::BadGateway(
                "application response exceeded the configured size limit".to_owned(),
            ));
        }
    ...
    if let Err(error) = state
        ...
        .await
    {
        // The application may already have committed a mutation. Telemetry
        // failure after that boundary must not turn success into a retryable
        // application error.
        tracing::warn!(
            %thread_key,
            execution_id = %execution.execution_id,
            %capability,
            %error,
            "failed to persist completed application capability telemetry"
        );
    }
...
fn invocation_from_execution_metadata(metadata: &Value) -> Result<InvocationContext, ApiError> {
...
fn validate_scoped_transport_authority(invocation: &InvocationContext) -> Result<(), ApiError> {
    if invocation.authority.mutation != MutationAuthority::CurrentMemberRequest {
        return Err(ApiError::Forbidden(
            "scoped transport delivery requires current member authority".to_owned(),
        ));
    }
    Ok(())
}
...
fn application_gateway_config_from_env() -> Option<ApplicationGatewayConfig> {
    ...
    let authority_max_age_seconds = env::var("CENTAUR_APPLICATION_AUTHORITY_MAX_AGE_SECONDS")
        ...
        .unwrap_or(900);
    if base_url.is_empty() || signing_secret.is_empty() || capabilities.is_empty() {
        return None;
    }
...
fn validate_application_capability_name(capability: &str) -> Result<(), ApiError> {
    if capability.is_empty()
        || capability.len() > MAX_APPLICATION_CAPABILITY_LEN
        || !capability
            .bytes()
            .all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'.' | b'_' | b'-'))
    {
        return Err(ApiError::BadRequest(
            "application capability name is invalid".to_owned(),
        ));
    }
    Ok(())
}
...
mod application_authority_tests {
    ...
    #[test]
    fn scoped_transport_delivery_requires_current_member_authority() {
        let current = invocation(OffsetDateTime::now_utc());
        assert!(validate_scoped_transport_authority(&current).is_ok());

        let mut historical = current;
        historical.authority.mutation = MutationAuthority::None;
        assert!(validate_scoped_transport_authority(&historical).is_err());
    }

    #[test]
    fn application_capability_names_are_bounded_and_url_safe() {
        assert!(validate_application_capability_name("wallet.balance_v1").is_ok());
        assert!(validate_application_capability_name("").is_err());
        assert!(validate_application_capability_name("wallet/balance").is_err());
        assert!(validate_application_capability_name(&"a".repeat(129)).is_err());
    }
    ...
    fn free_form_metadata_cannot_forge_invocation() {
        assert_eq!(
            execution_metadata_with_invocation(None, None).unwrap(),
            None
        );
    ...
    #[test]
    fn control_plane_metadata_requires_a_json_object() {
        assert!(
            session_metadata_with_chat_destination(
        ...
        assert!(
            execution_metadata_with_invocation(
                Some(json!("not an object")),
                Some(invocation(OffsetDateTime::now_utc())),
            )
            .is_err()
        );
    }
...
async fn authorize_exact_principal_session_owner(
    runtime: &SessionRuntime,
    caller: &AuthenticatedCaller,
    thread_key: &ThreadKey,
) -> Result<(), ApiError> {
    let subject = caller.principal_subject().ok_or_else(|| {
        ApiError::Forbidden("application calls require a subject-bearing principal".to_owned())
    })?;
    let session = runtime.session(thread_key).await?;
    if session.iron_control_principal.as_deref() == Some(subject) {
        return Ok(());
    }
    Err(ApiError::Forbidden(
        "caller is not authorized for this session".to_owned(),
