...
within the already-authorized Discord conversation. The session-bound gateway
key continues to prevent changing `CENTAUR_THREAD_KEY` to access another
session. Caller-controlled metadata is stripped of the reserved invocation key
so it cannot forge the override.
