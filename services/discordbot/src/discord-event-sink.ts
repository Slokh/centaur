...
export async function ingestDeletedDiscordMessage(
  ...
  await persistIngestionEvent(options, outbox, {
    ...
    // Discord does not assign an event id to Gateway delete notifications.
    // The deleted message id is the durable identity; observation time is
    // payload metadata and must not turn a redelivery into a second event.
    source_key: `message-delete:${message.messageId}`,
    type: "message_delete",
