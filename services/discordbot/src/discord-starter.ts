  ...
  for (let depth = 0; depth < 32; depth += 1) {
    ...
    if (embeddedParent && embeddedParent.id === parentId) {
      ...
      const currentChannelId =
        nonEmptyString(cursor.channel_id) ?? parsed.threadId ?? parsed.channelId;
      const channelId =
        nonEmptyString(reference?.channel_id) ??
        currentChannelId;
      // A bot token must never turn a visible reply reference into ambient
      // authority over another channel. Embedded snapshots remain usable.
      if (!channelId || !currentChannelId || channelId !== currentChannelId) break;
      try {
        const response = await fetchFn(
