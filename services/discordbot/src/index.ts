...
export function createDiscordbot(options: DiscordbotOptions): Discordbot {
  ...
  const ready = singleFlight(async () => {
    await Promise.all([
      ...
      ensureEventSinkOutboxConnected(options),
    ]);
  });
...
async function ensureEventSinkOutboxConnected(
  options: DiscordbotOptions,
): Promise<void> {
  const outbox = options.eventSinkOutbox;
  if (!outbox) return;
  for (let attempt = 0; ; attempt++) {
    try {
      await outbox.connect();
      if (attempt > 0) {
        traceLog(options, "discordbot_event_sink_outbox_connected", undefined, {
          attempts: attempt + 1,
        });
      }
      return;
      ...
      traceLog(options, "discordbot_event_sink_outbox_connect_retry", undefined, {
        attempt: attempt + 1,
        delay_ms: delayMs,
        error: errorMessage(error),
      });
...
async function isStateHealthy(
  state: StateAdapter,
  outbox: DiscordbotOptions["eventSinkOutbox"],
): Promise<boolean> {
  try {
    await Promise.all([
...
function observeDiscordEventSink(
  ...
  backgroundWaitUntil(
    promise.catch((error) => {
      traceLog(options, "discordbot_event_sink_enqueue_failed", undefined, {
        error: errorMessage(error),
...
function rendererOptions(
  ...
  return {
    ...mapper,
    mapPublicError: mapper?.mapPublicError ?? discordPublicExecutionError,
...
function discordPublicExecutionError(error: string): string {
  ...
  if (
    /^The requested model '[A-Za-z0-9._/-]+' does not exist\.?$/.test(
      validationMessage,
    )
  ) {
  ...
  if (/\b402\b|payment required|insufficient credits?|weekly limit|can only afford/i.test(normalized)) {
    return "The model provider rejected the request. Please try again after its account limits are resolved.";
  }
