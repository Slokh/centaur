...
export type DiscordReplayContext = {
  channel_id: string;
  guild_id: string;
  messages: DiscordReplayMessage[];
  source_message_id: string;
  target_message: DiscordReplayMessage;
...
export async function resolveDiscordReplayContext(input: {
  ...
  for (let depth = 0; depth < 50; depth += 1) {
    ...
    if (parentChannelId !== parsed.channelId) break;
  ...
  return {
    ...
    messages: replayMessages.map((message) =>
      toReplayMessage(message, input.applicationId),
    ),
    source_message_id: source.id,
    target_message: toReplayMessage(target, input.applicationId),
...
function toReplayMessage(
  message: DiscordMessage,
  applicationId: string,
): DiscordReplayMessage {
  return {
    author_name:
      ...
      message.author.id,
    id: message.id,
    role: message.author.id === applicationId ? "assistant" : "user",
