...
export type DiscordEventSinkClaim = {
  attempt: number;
  leaseToken: string;
...
export interface DiscordEventSinkOutbox {
  acknowledge(claim: DiscordEventSinkClaim): Promise<boolean>;
  claim(batchSize: number, leaseMs: number): Promise<DiscordEventSinkClaim[]>;
 ...
 */
export class PostgresDiscordEventSinkOutbox
implements DiscordEventSinkOutbox {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (!versions.has(1)) {
        await client.query(`
          CREATE TABLE IF NOT EXISTS discord_event_sink_outbox (
        ...
        `);
        await client.query(`
          CREATE INDEX IF NOT EXISTS discord_event_sink_outbox_ready_idx
          ON discord_event_sink_outbox
      ...
      if (!versions.has(2)) {
        await client.query(`
          ALTER TABLE discord_event_sink_outbox
            ADD COLUMN IF NOT EXISTS dead_lettered_at TIMESTAMPTZ,
            ADD COLUMN IF NOT EXISTS dead_letter_error TEXT
        `);
        await client.query(`
          INSERT INTO discordbot_schema_migrations(component, version)
  ...
  async enqueue(
    ...
    if (Buffer.byteLength(serialized) > this.maxPayloadBytes) {
      throw new Error("Discord event sink payload exceeds durable queue limit");
    }
    await this.pool.query(
      `
        INSERT INTO discord_event_sink_outbox
  ...
  async claim(
    batchSize: number,
    leaseMs: number,
  ): Promise<DiscordEventSinkClaim[]> {
    if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 1_000) {
      throw new Error("Discord event sink batch size must be between 1 and 1000");
    }
    if (!Number.isInteger(leaseMs) || leaseMs < 1_000) {
      throw new Error("Discord event sink lease must be at least one second");
    }
    ...
    const result = await this.pool.query<{
      ...
      `
        WITH candidates AS (
          SELECT source_key
          FROM discord_event_sink_outbox
          WHERE key_prefix = $1
        ...
        UPDATE discord_event_sink_outbox AS outbox
  ...
  async acknowledge(claim: DiscordEventSinkClaim): Promise<boolean> {
    const result = await this.pool.query(
      `
        DELETE FROM discord_event_sink_outbox
  ...
  async retry(
    ...
    if (claim.attempt >= MAX_DELIVERY_ATTEMPTS) {
      const result = await this.pool.query(
        `
          UPDATE discord_event_sink_outbox
          SET dead_lettered_at = NOW(),
              dead_letter_error = LEFT($4, 500),
              leased_until = NULL,
              lease_token = NULL,
              updated_at = NOW()
          WHERE key_prefix = $1 AND source_key = $2 AND lease_token = $3
        `,
        [this.keyPrefix, claim.sourceKey, claim.leaseToken, error],
      );
      return result.rowCount === 1;
    }
  ...
  async pendingCount(): Promise<number> {
    const result = await this.pool.query<{ count: string }>(
      `
        ...
        FROM discord_event_sink_outbox
        WHERE key_prefix = $1 AND dead_lettered_at IS NULL
      `,
      [this.keyPrefix],
    );
  ...
  async deadLetterCount(): Promise<number> {
    const result = await this.pool.query<{ count: string }>(
      `
        SELECT COUNT(*)::text AS count
        FROM discord_event_sink_outbox
        WHERE key_prefix = $1 AND dead_lettered_at IS NOT NULL
      `,
      [this.keyPrefix],
    );
    return Number.parseInt(result.rows[0]?.count ?? "0", 10);
...
export class MemoryDiscordEventSinkOutbox
implements DiscordEventSinkOutbox {
  ...
  async healthCheck(): Promise<void> {}
  async deadLetterCount(): Promise<number> {
    return [...this.rows.values()].filter((row) => row.deadLettered).length;
  }
