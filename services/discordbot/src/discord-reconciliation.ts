...
const CHECKPOINT_PREFIX = "discordbot:event-sink:channel-checkpoint:";
const BACKFILL_PREFIX = "discordbot:event-sink:channel-backfill:";
const BACKFILL_COMPLETE = "complete";
const ARCHIVED_THREAD_CURSOR_PREFIX = "discordbot:event-sink:archived-thread-cursor:";
const ARCHIVED_THREAD_SEEN_PREFIX = "discordbot:event-sink:archived-thread-seen:";
const CHANNEL_SNAPSHOT_PREFIX = "discordbot:event-sink:channel-snapshot:";
const PAGE_LIMIT = 10;
 ...
 * normalized messages have been durably queued for event-sink delivery.
 */
export async function reconcileDiscordArchive(
  options: DiscordbotOptions,
  state: StateAdapter,
): Promise<number> {
  if (!options.eventSinkUrl || !options.eventSinkToken) return 0;
  let observed = 0;
  for (const guildId of options.guildAllowlist ?? []) {
    ...
    const counts = await mapConcurrent(
      ...
      Math.min(
        ...
        Math.max(
          1,
          options.eventSinkArchiveReconciliationConcurrency ??
