...
const options: DiscordbotOptions = {
  ...
  eventSinkArchiveReconciliationConcurrency: optionalNumberEnv(
    "DISCORDBOT_EVENT_SINK_ARCHIVE_RECONCILIATION_CONCURRENCY",
  ),
  eventSinkArchiveReconciliationEnabled:
    optionalEnv("DISCORDBOT_EVENT_SINK_ARCHIVE_RECONCILIATION_ENABLED") ===
    "true",
  eventSinkArchiveReconciliationIntervalMs: optionalSecondsEnv(
...
const reconcileArchive = singleFlight(async () => {
  try {
    ...
    if (pending >= archiveBacklogHighWatermark) {
      log("warn", "discordbot_event_sink_archive_backpressured", {
    ...
    if (observed > 0) {
      log("info", "discordbot_event_sink_archive_reconciled", {
        observed_count: observed,
      });
    }
  } catch (error) {
    log("error", "discordbot_event_sink_archive_reconciliation_failed", {
      error: errorMessage(error),
    });
  }
});
...
if (options.eventSinkArchiveReconciliationEnabled === true) {
  triggerArchive();
  archiveTimer = setInterval(
    triggerArchive,
    Math.max(
      1_000,
      options.eventSinkArchiveReconciliationIntervalMs ?? 60_000,
    ),
  );
  archiveTimer.unref();
}
