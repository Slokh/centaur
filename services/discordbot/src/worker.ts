...
import { recoverApplicationIngestion } from "./discord-ingestion";
...
const outbox = new PostgresApplicationIngestionOutbox({
...
const options: DiscordbotOptions = {
  ...
  applicationIngestionDeliveryTimeoutMs: optionalNumberEnv(
    "DISCORDBOT_APPLICATION_DELIVERY_TIMEOUT_MS",
  ),
  applicationIngestionOutbox: outbox,
  applicationIngestionRecoveryBatchSize: optionalNumberEnv(
    "DISCORDBOT_APPLICATION_RECOVERY_BATCH_SIZE",
  ),
  applicationIngestionRecoveryConcurrency: optionalNumberEnv(
    "DISCORDBOT_APPLICATION_RECOVERY_CONCURRENCY",
  ),
  applicationIngestionRecoveryLeaseMs: optionalNumberEnv(
    "DISCORDBOT_APPLICATION_RECOVERY_LEASE_MS",
  ),
  applicationIngestionToken: requiredEnv(
    "DISCORDBOT_APPLICATION_INGESTION_TOKEN",
  ),
  applicationIngestionUrl: requiredEnv(
...
const recoveryIntervalMs = optionalNumberEnv(
  "DISCORDBOT_APPLICATION_RECOVERY_INTERVAL_MS",
) ?? 1_000;
const archiveBacklogHighWatermark = optionalNumberEnv(
  "DISCORDBOT_APPLICATION_ARCHIVE_BACKLOG_HIGH_WATERMARK",
) ?? 10_000;
...
const recoverOutbox = singleFlight(async () => {
  try {
    const result = await recoverApplicationIngestion(options, outbox);
    if (result.claimed > 0 || result.failed > 0) {
      log("info", "discordbot_application_ingestion_recovered", result);
    }
  } catch (error) {
    log("error", "discordbot_application_ingestion_recovery_failed", {
      error: errorMessage(error),
    });
  }
});
...
const app = new Hono();
app.get("/live", (c) =>
  c.json({ ok: true, service: "discordbot-ingestion-worker" }),
);
const readiness = async (c: Context) => {
  ...
  return c.json(
    {
      ok: stateReady,
      service: "discordbot-ingestion-worker",
      state: stateReady,
    },
    stateReady ? 200 : 503,
  );
...
log("info", "discordbot_ingestion_worker_started", { port: server.port });
...
if (options.applicationArchiveReconciliationEnabled !== false) {
  triggerArchive();
...
const shutdown = async (signal: string): Promise<void> => {
  ...
  log("info", "discordbot_ingestion_worker_shutdown_started", { signal });
  ...
  server.stop();
  log("info", "discordbot_ingestion_worker_shutdown_complete", { signal });
  process.exit(0);
};
...
async function connectDurableState(): Promise<void> {
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      log("warn", "discordbot_ingestion_worker_postgres_connect_retry", {
        attempt: attempt + 1,
        delay_ms: delayMs,
        error: errorMessage(error),
      });
