...
describe("Discord event sink", () => {
  ...
  it("durably retries an event after the sink is unavailable", async () => {
  ...
  it("deduplicates repeated message delete observations", async () => {
    const outbox = new MemoryDiscordEventSinkOutbox();
    const options = {
      apiUrl: "http://centaur",
      applicationId: "app",
    ...
    await ingestDeletedDiscordMessage(options, {
      guildId: "G1",
      channelId: "C1",
      messageId: "M-delete",
    ...
    await ingestDeletedDiscordMessage(options, {
      ...
      channelId: "C1",
      messageId: "M-delete",
      deletedAt: "2026-08-10T10:00:01.000Z",
    });

    expect(await outbox.pendingCount()).toBe(1);
    const [claim] = await outbox.claim(1, 1_000);
    expect(claim?.sourceKey).toBe("message-delete:M-delete");
    expect(claim?.payload.deleted_at).toBe("2026-08-10T10:00:00.000Z");
  });

  it("does not wait for slow sink delivery after writing the outbox", async () => {
    const outbox = new MemoryDiscordEventSinkOutbox();
    await outbox.connect();
